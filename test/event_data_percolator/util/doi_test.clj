...
(deftest ^:unit resolve-escaped
  (testing "resolve-doi-maybe-escaped should return an unescaped DOI, if input unescaped and it's valid"
    (fake/with-fake-http ["https://doi.org/api/handles/10.1007/s00423-015-1364-1" (util/doi-ok "10.1007/s00423-015-1364-1")]
      (is (= (doi/resolve-doi-maybe-escaped util/mock-context "10.1007/s00423-015-1364-1")
  ...
  (testing "resolve-doi-maybe-escaped should return an unescaped DOI, if input URL escaped and it's valid"
    (fake/with-fake-http ["https://doi.org/api/handles/10.1007/s00423-015-1364-1" (util/doi-ok "10.1007/s00423-015-1364-1")]
      (is (= (doi/resolve-doi-maybe-escaped util/mock-context "10.1007%2fs00423-015-1364-1")
               "10.1007/s00423-015-1364-1")
...
(deftest ^:unit qmarks-hash-not-included-resolve-doi
  (testing "Trailing question marks and hashes are discarded by resolve-doi"
    (is (= (doi/resolve-doi util/mock-context "10.1111/nicc.12290#.wlw5yueemak.twitter")
           "10.1111/nicc.12290"))

    (is (= (doi/resolve-doi util/mock-context "10.2752/136270497779613666?journalcode=rfft20")
    ...
    (is (= (doi/resolve-doi util/mock-context "10.1007/s11739-017-1643-7?utm_content=bufferd3cda&utm_medium=social&utm_source=twitter.com&utm_campaign=buffer")
           "10.1007/s11739-017-1643-7"))

    ; https://github.com/CrossRef/event-data-percolator/issues/25
    (is (= (doi/resolve-doi util/mock-context "10.1007/s00127-017-1346-4?wt_mc=internal.event.1.sem.articleauthoronlinefirst")
           "10.1007/s00127-017-1346-4"))))

(deftest ^:unit shortdoi-resolve-doi
  (testing "ShortDOIs are resolved to normal DOIs by resolve-doi"
    (is (= (doi/resolve-doi util/mock-context "hvx")
              "10.5555/12345678"))))
...
(deftest ^:unit empty-doi-resolve-doi
  (testing "An empty DOI doesn't resolve as extant"
    (is (= (doi/resolve-doi util/mock-context "") nil))
    (is (= (doi/validate-doi-dropping util/mock-context "") nil))
    (is (= (doi/validate-doi-dropping util/mock-context "https://doi.org/") nil))))
...
(deftest ^:unit nonexistent-doi-resolve-doi
  (testing "Nonexisting DOIs that return 303 from proxy shouldn't be accepted as extant."
    (is (= (doi/resolve-doi util/mock-context "www.uclouvain.be/784506.html") nil))
    (is (= (doi/validate-doi-dropping util/mock-context "https://doi.org/www.uclouvain.be/784506.html") nil))))
