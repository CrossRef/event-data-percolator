...
(deftest process-url-observation
  ...
  (testing "URL DOIs on dx.doi.org can be matched"
    (let [result (url/process-url-observation {:type "url" :input-url "http://dx.doi.org/10.5555/1111"} domain-set)]
      (is (= result {:type "url"
  ...
  (testing "ShortDOI URL DOIs can be extracted from text"
    (let [result (url/process-url-observation {:type "url" :input-url "http://doi.org/abcd"} domain-set)]
      (is (= result {:type "url"
  ...
  (testing "Landing Page URLs can be extracted from text"
    (let [result (url/process-url-observation {:type "url" :input-url "http://example.com/four"} domain-set)]
      (is (= result {:type "url"
  ...
  (testing "Landing Page URLs not on recognised domain list are not extracted."
    (let [result (url/process-url-observation {:type "url" :input-url "http://bad-example.com/four"} domain-set)]
      (is (= result {:type "url"
                     :input-url "http://bad-example.com/four"
                     :candidates []}))))

  (testing "Nil input handled ok."
    (let [result (url/process-url-observation {:type "url" :input-url nil} domain-set)]
      (is (= result {:type "url"
                     :input-url nil
                     :candidates []})))))
