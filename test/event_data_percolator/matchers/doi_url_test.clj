...
(deftest ^:component match-doi-url-candidate
  (testing "match-doi-url-candidate matches valid DOI."
    (fake/with-fake-http ["https://doi.org/api/handles/10.5555/12345678" {:status 200 :body (json/write-str {"handle" "10.5555/12345678"})}]
      (let [result (doi-url/match-doi-url-candidate util/mock-context {:value "https://doi.org/10.5555/12345678"})]
        (is (= result {:value "https://doi.org/10.5555/12345678"
  ...
  (testing "match-doi-url-candidate does not match nonexistent DOI."
    ...
    (fake/with-fake-http [#"https://doi.org/api/handles/10.5555/12" (util/doi-not-found)
      ...
      (let [result (doi-url/match-doi-url-candidate util/mock-context {:value "http://doi.org/10.5555/12345678"})]
        (is (= result {:value "http://doi.org/10.5555/12345678"
                       :match nil
                       :method :doi-literal
                       :verification :literal})))))
  ...
  (testing "match-doi-url-candidate normalizes DOI."
    (fake/with-fake-http ["https://doi.org/api/handles/10.5555/12345678" (util/doi-ok "10.5555/12345678")]
      ...
      (let [result (doi-url/match-doi-url-candidate util/mock-context {:value "http://dx.doi.org/10.5555/12345678"})]
        (is (= result {:value "http://dx.doi.org/10.5555/12345678"
                       :match "https://doi.org/10.5555/12345678"
                       :method :doi-literal
                       :verification :literal}))))))
